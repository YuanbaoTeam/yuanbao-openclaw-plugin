...
| 项 | 现状 |
|---|---|
| 运行器 | Node 原生 `node --test` + `tsx`，`pnpm test` 跑 **192 用例 ≈ 1s** |
| 已有单测 | 29 个 `*.test.ts`：`pipeline/middlewares/*`、`messaging/handlers/*`、`utils`、`infra/cache/ttl-db`、`quote/extract/targets/chat-history` |
| 集成测试 | ⚠️ 仅单中间件粒度，无 inbound→reply / outbound / WS 端到端 |
...
| 覆盖率度量 | ❌ 完全没有。实测真实基线（`c8 --all`）：**Lines 38.32% / Functions 43.14% / Branches 70.84%**（近半代码未被任何测试导入） |
| CI | `ci.yml`：lint / test / build 三段 ✅ |
| pre-commit | `.githooks/pre-commit` 跑 lint + test ✅ |
...
| 场景 | 构造 | 断言 |
|---|---|---|
| inbound | 原始 `msg_body`（文本/图片/@机器人/引用）→ 跑完整 pipeline | `route` / `rewrittenBody` / 是否触发 reply；覆盖 c2c 与 group+requireMention |
| outbound | `actions/deliver` + `queue` + mock `wsClient.send` | 发包序列（streaming / merge-text 下） |
| WS gateway | mock socket | 连接 / 心跳 / 断线重连 / 收帧后正确派发到 pipeline |
...
**允许 exclude 的低收益胶水代码**（不计入分母，需在 `.c8rc` 显式列出并注释理由）：
纯 barrel `index.ts`、`logger.ts`、`*.proto` / `*.json`、类型声明。
其余一律计入——包括 `channel.ts` / `runtime.ts` / `setup-*`（这些有逻辑，要测）。
...
- **保持 node 原生 + tsx**，不引入 vitest/jest（192 用例已基于它，迁移无收益）。唯一新增 `c8`。
...
- [ ] **T0.1** 安装 `c8`；新增 `.c8rc.json`：`all:true`、`src:["src"]`、`exclude`（barrel `index.ts`/`logger.ts`/`*.proto`/`*.json`/类型声明，逐条注释理由）、`check-coverage:true` + 四项阈值=当前真实值（lines/statements 38、functions 43、branches 70）
- [ ] **T0.2** `package.json` 增 `test:coverage`（带 `--all`）
- [ ] **T0.3** CI `test` job 改跑 `test:coverage` 并卡阈值；故意删一个测试验证 CI 会红

### Phase 1 · 签票 / codec / 纯函数（~2~3d，零·轻 mock）

- [ ] **T1.1** `access/ws/conn-codec.test.ts`：round-trip + 异常字节不崩 + `PROTO-001` golden（含 authbind/ping/ack 构造）
- [ ] **T1.2** `access/ws/biz-codec.test.ts`：各 `encode*/decode*` round-trip + 异常字节
- [ ] **T1.3** `access/http/request.test.ts`：**签票** / `computeSignature` / ticket，固定输入断言 + `SIGN-001` golden
- [ ] **T1.4** 纯函数补满：`outbound/queue` · `utils/markdown` · `commands/upgrade/utils`(semver) · `commands/log-upload/extractor` · `infra/reply-classify`
- [ ] **T1.5** `accounts` · `infra/env` · `infra/cos` 纯逻辑分支（IO mock）
- [ ] **T1.6** 抬高 `.c8rc` 阈值到 Phase 1 实测值

### Phase 2 · WS 建联 + 5 条链路集成（~3~4d，mock socket）

- [ ] **T2.1** 链路1 入站 pipeline：C2C 文本 / 群聊@ / 群聊未@(requireMention) / 引用 / 媒体 / skipSelf / 命令 七变体
- [ ] **T2.2** 链路3 WS 建联：mock socket 测 authbind 握手 → connected、心跳（假定时器）、断线重连退避
- [ ] **T2.3** 链路3 收帧派发：`gateway` decode → `wsPushToInboundMessage` → 接入 pipeline 的端到端冒烟
- [ ] **T2.4** 链路2 出站：`handleAction` + `queue` + mock `send`，merge-text / immediate 发包序列 + sticker/media/text 分发
- [ ] **T2.5** 链路4 dispatcher：session-queue 串行/并发 + abort 中止 + debounce 合并（假定时器）
- [ ] **T2.6** 链路5 群成员回填：`recordMember` + `infra/cache/member` + `messaging/directory`
- [ ] **T2.7** 抬高 `.c8rc` 阈值到 Phase 2 实测值

### Phase 3 · 收尾到 90%（~1~2d）

- [ ] **T3.1** 扫 `c8 --all` 剩余 <90% 文件逐个补齐（handlers `file`/`video`/`sound`、`directory`、`link-card`、`trace/context`、`guard-*` 分支）
- [ ] **T3.2** `.c8rc` 四项阈值锁定 **90**，CI 卡死；README/CLAUDE 注明覆盖率门禁
