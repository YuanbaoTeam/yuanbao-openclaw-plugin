...
void test("recordMediaHistory appends entries", () => {
  recordMediaHistory("group:g-1", { sender: "u", timestamp: 1, medias: [{ mediaType: "image", url: "http://a" }] });
  recordMediaHistory("group:g-1", { sender: "u", timestamp: 2, medias: [{ mediaType: "image", url: "http://b" }] });
...
void test("recordMediaHistory evicts oldest beyond the per-chat cap (50)", () => {
  for (let i = 0; i < 55; i++) {
    recordMediaHistory("group:g-1", { sender: "u", timestamp: i, medias: [{ mediaType: "image", url: `http://${i}` }] });
  }
