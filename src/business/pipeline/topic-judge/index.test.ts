...
void test("shouldBotReplyInTopic 无 soul 内容 → no soul configured", async () => {
  const r = await shouldBotReplyInTopic({ topicId: "t1", rawBody: "帮我", soul: "" });
  assert.equal(r.shouldReply, false);
  assert.equal(r.reason, "no soul configured");
});

void test("shouldBotReplyInTopic soul 有内容但无 Reply Rules 段 → no rule matched", async () => {
...
void test("shouldBotReplyInTopic Reply Rules 段为空 → no rule matched", async () => {
...
void test("shouldBotReplyInTopic 完全损坏的 soul 也不抛错", async () => {
  // 用一段完全不像 markdown 的乱码 —— 应该走 "no rule matched" 分支
  const r = await shouldBotReplyInTopic({
    topicId: "t1",
    rawBody: "报名",
