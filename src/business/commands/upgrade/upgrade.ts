...
async function runSpecifiedVersionFlow(params: {
  ...
  const targetVersion = _targetVersion ?? (await getLatestStableVersion());
...
export async function performUpgrade(
  ...
  // Use `--deep` so that environments without systemd-user / launchd still get a real
  // kill → respawn cycle. Fall back to plain `restart` if `--deep` is unsupported.
  let restartResult = await runOpenClawCommand(["gateway", "restart", "--deep"]);
  if (!restartResult.ok) {
    const combinedErr = `${restartResult.error ?? ""}\n${restartResult.stderr ?? ""}`;
    const flagUnsupported = /unknown option|unrecognized option|--deep/i.test(combinedErr);
    if (flagUnsupported) {
      log.warn("gateway restart --deep 不受支持，回退到普通 restart", {
  ...
  // If restart did not actually swap the process (e.g. systemd-user unavailable inside a
  ...
  if (!restartResult.ok || supervisorUnavailable) {
    ...
    // return "⚠️ 升级文件已就位，但 Gateway 未自动重启。请由运维手动重启 Gateway 后升级才会生效（容器环境请确保 supervisor 或 `docker restart` 能托管 gateway 进程）。";
  }
