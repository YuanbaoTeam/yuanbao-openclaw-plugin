...
function createMemberMemoryTool(ctx: OpenClawPluginToolContext) {
  ...
  return {
    ...
    description:
      ...
      + "When no userId is given, the action targets the current sender."
    ...
    async execute(toolCallId: string, params: Record<string, unknown>) {
      ...
      if (!target) {
        // Note: do NOT advertise `nickname` as a lookup target — it is display
        // only. To target another member, resolve a userId via
        // query_session_members first.
        return json({
          success: false,
          msg: "Could not resolve the target member. Pass a `userId` (resolve a name via query_session_members first if needed), or call from a group session as a member.",
        });
      }
