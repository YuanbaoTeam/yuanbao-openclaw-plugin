  ...
  schema: {
    ...
    properties: {
      ...
      botLoop: {
        type: "object",
        title: "BOT 循环阻断",
        description: "群聊中检测到其他 BOT 反复触发本 BOT 时的自动静音策略，仅群聊生效",
        properties: {
          enabled: {
            type: "boolean",
            title: "启用循环阻断",
            description: "关闭后即使检测到 BOT 互相触发也不会静音",
            default: true,
          },
          threshold: {
            type: "integer",
            title: "触发阈值（条）",
            description: "滑动窗口内接收来自其他 BOT 的消息达到该条数即进入静音",
            minimum: 1,
            default: 5,
          },
          windowMs: {
            type: "integer",
            title: "统计窗口（毫秒）",
            description: "滑动窗口长度，超过该时长未再触发则计数重置",
            minimum: 0,
            default: 600000,
          },
          muteMs: {
            type: "integer",
            title: "静音时长（毫秒）",
            description: "触发阈值后静音时长，期间所有入站消息静默丢弃",
            minimum: 0,
            default: 1800000,
          },
        },
